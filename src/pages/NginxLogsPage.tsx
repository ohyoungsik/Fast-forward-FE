...
export default function NginxLogsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return mockNginxLogs;
    return mockNginxLogs.filter((r) => {
  ...
  return (
    <div className="space-y-8">
      <header>
        <h2 className="text-3xl font-extrabold text-white tracking-tight">Nginx 로그</h2>
        <p className="text-gray-500 mt-2">상태코드/에러 메시지 기반으로 Reverse Proxy 이벤트를 탐색합니다.</p>
      </header>
